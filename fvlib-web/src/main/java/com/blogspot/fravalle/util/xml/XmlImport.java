...
public class XmlImport {
	...
	public static final void main(String args[]) {
		// System.out.println(replaceEntity("La gravità è una forza più forte dell'assenza, del perché ne parlerò 'Cosimo de Cosmis'"));
		...
		try {
			...
			while(rs.next()) {
				try {
					...
					System.out.println("{ERROR-MESSAGE-DETAIL}: --> " + e.getMessage());
				} finally {
		...
		try {
			...
			while(rs.next()) {
				...
				try {
					...
					System.out.println("{XML-PATH}: --> " + sFileIn);
					System.out.println("{ERROR-ON}: --> " + detailExportId);
					System.out.println("{MESSAGE-DETAIL}: --> " + e.getMessage());
				} finally {
					Runtime.getRuntime().gc();
				}
			}
		...
		try {	
			...
			for (int e=0; e < articleNodes.getLength(); e++) {
				...
				if (articleNodes.item(e).getNodeName().equals("CONTENT")) {
					System.out.println( nodeValue( articleNodes.item(e) ) );
					textContainer += nodeValue( articleNodes.item(e) );
				}
			}
		
		} catch (Exception ex) {
			System.out.println("<P><B>error: </B><I>"+ ex.getMessage() + "</I> <B>" + "[" + statusError + ", " + 0 + "]" + "</B></P>");
		}
	}
